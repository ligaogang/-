...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		buildSearchBox = function (index, top, left, parent) {
			...
			str1 += "</div>";
		    //MODIFY: 重置所有
			str1 += "<div class='search_buttons' style='width:100%'><a tabindex='0' id='bs_resetall' class='fm-button " + common.button + " ui-search'>" + "&#37325;&#32622;&#25152;&#26377;&#26465;&#20214;" + "</a></div>";
		    //MODIFY: 重置所有 END
			elem.append(str1);
			...
			$("#bs_reset, #bs_search", "#search_menu", "#bs_resetall").hover(function () {
				$(this).addClass(hover);
			},function(){
				$(this).removeClass(hover);
			});
		    ...
		    //MODIFY: 重置所有
			$("#bs_resetall", elem).on('click', function (e) {
			    ts.p.colFilters[cm.name] = {};
			    for (var i in ts.p.colFilters) {
			        ts.p.colFilters[i] = {};
			    }
		    ...
		    //MODIFY 隐藏 搜索框
			if (cm.sorttype == undefined || cm.sorttype === 'text') {
			    elem.find("#operand").parent().hide();
			    elem.find("#oper2").parent().hide();
			    elem.find("[id^=sval2_]").parent().hide();
			}
		    //MODIFY 隐藏 搜索框 END
		},
			...
			if (o.useColSpanStyle) {
				...
				$htable.find("span.ui-jqgrid-resize").each(function () {
					...
					if ($parent.is(":visible")) {
					    if (typeof IsIE9 != "undefined" && IsIE9) {
					        this.style.cssText = 'height: ' + $parent.height() + 'px; cursor: col-resize;';
					    } else {
